...
- (void)viewDidLoad {
    ...
    _calendarViewHeightConstraint.constant = floorf(self.view.bounds.size.width / 7.0f) * 3.0f + WEEKDAY_VIEW_HEIGHT;
...
- (void)viewDidAppear:(BOOL)animated{
    ...
    [self.calendarView scrollCalendarToDate:[NSDate date] fromAgendaView:NO animated:NO];
...
- (void)calendarView:(ASCalendarView *)calendarView scrolledToSectionOfDifferentNumberOfRows:(NSInteger)rows {
    if (_calendarViewHeightConstraint.constant == floorf(self.view.bounds.size.width / 7.0f) * rows + WEEKDAY_VIEW_HEIGHT) {
        return;
    }
...
- (void)agendaView:(ASAgendaView *)agendaView didScrollToDate:(NSDate *)date {
    [self.calendarView scrollCalendarToDate:date fromAgendaView:YES animated:YES];
    self.titleView.text = [date monthYearString];
}

- (void)agendaView:(ASAgendaView *)agendaView stoppedScrolling:(BOOL)stoppedScroll {
    [self.calendarView agendaViewStoppedScrolling];
}

- (void)agendaView:(ASAgendaView *)agendaView scrolledFast:(BOOL)fastScroll {
    [self.calendarView agendaViewScrolledFast];
}
...
+ (NSArray *)calendarAgendas {
    ...
    for (NSInteger i = 0; i < 120; i++) {
        ...
        NSString *agendaDescription = [[self class] agendaDescriptions][rand() % 11];
        NSDictionary *dict = @{
                               @"startDate": startDate,
                               @"endDate": endDate,
