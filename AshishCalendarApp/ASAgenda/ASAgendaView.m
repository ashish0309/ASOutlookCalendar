...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    if ([self.delegate respondsToSelector:@selector(agendaView:stoppedScrolling:)]) {
        [self.delegate agendaView:self stoppedScrolling:YES];
    }
}

- (void)scrollViewWillEndDragging:(UIScrollView *)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(inout CGPoint *)targetContentOffset{
    // Get at least 2 visible rows for the targetcontentoffset.
    NSArray *visiblePaths = [self.tableView indexPathsForRowsInRect:CGRectMake(0, targetContentOffset->y, 100, scrollView.bounds.size.height)];
    if (visiblePaths.count < 2) {
        return;
    }
    ...
    /**
     Show overlay view in Calendar view when scroll is fast that is more
     than 1000 points.
     */
    if (ABS(scrollView.contentOffset.y - targetContentOffset->y) > 1000) {
        if ([self.delegate respondsToSelector:@selector(agendaView:scrolledFast:)]) {
            [self.delegate agendaView:self scrolledFast:YES];
        }
    }
}
